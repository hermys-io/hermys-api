...
@pytest.mark.anyio
async def test_get_or_rise_clerk_by_id(shared_db: AsyncIOMotorDatabase):
    ...
    result = await repo.get_or_rise(by='_id', value=clerk_id)
...
@pytest.mark.anyio
async def test_get_or_rise_clerk_by_slug(shared_db: AsyncIOMotorDatabase):
    clerk_slug = 'my-clerk'
    repo = ClerkRepository(db=shared_db)
    dbo = ClerkDBO(
        slug=clerk_slug,
    ...
    result = await repo.get_or_rise(by='slug', value=clerk_slug)
...
@pytest.mark.anyio
async def test_get_or_rise_clerk_return_none_if_not_found(
    shared_db: AsyncIOMotorDatabase,
):
    repo = ClerkRepository(db=shared_db)

    with pytest.raises(ClerkNotFound):
        await repo.get_or_rise(by='slug', value='unknown-slug')
